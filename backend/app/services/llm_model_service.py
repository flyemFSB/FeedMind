...
def _to_model_read(row: LLM) -> LLMModelRead:
...
class LLMModelService:
    ...
    def get_selected_llm_model(self) -> SelectedModelRead:
        ...
        with get_session() as session:
            repository = LLMRepository(session)
            model_id = repository.get_selected_id()
    ...
    def set_selected_llm_model(self, payload: SelectedModelUpdate) -> SelectedModelRead:
        ...
        with get_session() as session:
            repository = LLMRepository(session)
            model = repository.get_llm(payload.id)
            if model is None:
                logger.warning("设置默认模型失败，模型不存在 id={}", payload.id)
                raise ModelConfigNotFoundError

            repository.clear_selected()
            repository.set_selected(model)
