  ...
  return (
    <div className="flex h-full flex-col bg-white">
      <div className="border-b border-[#e8e8ed] px-6 py-4">
        <h2 className="text-[15px] font-semibold text-[#1d1d1f] mb-3">Wiki Search</h2>
      ...
      <div className="flex-1 overflow-y-auto">
        {searched && results.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-[13px] text-[#86868b]">No results for "{query}"</p>
            <p className="text-[11px] text-[#d2d2d7] mt-1">Try different keywords</p>
          </div>
        )}
        {results.length > 0 && (
            ...
            {results.map((result, i) => (
              ...
              >
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  {result.titleMatch && <span className="text-[10px] text-[#0071e3] bg-[#e8f0fe] rounded px-1.5 py-0.5">Title</span>}
                </div>
                <p className="text-[11px] text-[#86868b] leading-relaxed">{result.snippet}</p>
                <p className="text-[10px] text-[#d2d2d7] mt-0.5">{result.path} · Score: {result.score.toFixed(1)}</p>
              </button>
            ))}
          </div>
        )}
        {!searched && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-[13px] text-[#86868b]">Search across all wiki pages</p>
          </div>
        )}
      </div>
    </div>
